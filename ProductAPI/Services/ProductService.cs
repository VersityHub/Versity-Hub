...
namespace VersityHub.VersityHubWebAPI.Product.Services
{
    public class ProductService : IProductService
    {
        ...
        public async Task<ProductModel> GetProductByTitleAsync(string productTitle)
        {
            var product = await _context.Products.FirstOrDefaultAsync(p => p.Title.Equals(productTitle, StringComparison.OrdinalIgnoreCase)); 
            return _mapper.Map<ProductModel>(product);
        }
        public async Task<ProductModel> GetProductByCategoryAsync(string productCategory)
        {
            var product = await _context.Products.FirstOrDefaultAsync(p => p.Category.Equals(productCategory, StringComparison.OrdinalIgnoreCase));
            return _mapper.Map<ProductModel>(product);
        }
